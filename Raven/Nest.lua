...
local function SplashEffect(bar, anchor1, frame, anchor2, xoffset, yoffset)
	local tex = bar.iconTexture:GetTexture()
	if not tex then
		return
	end
	...
	if b then
		...
		scale:SetDuration(0.65)
		scale:SetOrder(1)
		local alpha = b.anim:CreateAnimation("Alpha")
		alpha:SetFromAlpha(1)
		alpha:SetToAlpha(0) -- LEGION change
	...
	b.texture:SetAllPoints(b.frame)
	b.texture:Show()
	b.anim:Stop()
	b.anim:Play()
...
local function UpdateSplashAnimations()
	...
	for k, b in pairs(splashAnimations) do
		if now > b.endTime then
			b.anim:Pause()
			splashAnimations[k] = nil
			splashAnimationPool[b] = true
...
local function FlashEffect(bar)
	...
	if anim.maxAlpha ~= bar.alpha or anim.minAlpha ~= bar.minAlpha or anim.flashPeriod ~= bar.flashPeriod then
		...
		anim.maxAlpha = bar.alpha
		anim.minAlpha = bar.minAlpha
		anim.flashPeriod = bar.flashPeriod
	end
	...
	if not a then -- allocate an animation if necessary
		...
		if a then
			...
			for i = 1, 8 do -- create sparklers
				...
				x = a.anim:CreateAnimation("Alpha")
				x:SetTarget(animTarget)
				x:SetFromAlpha(0)
				x:SetToAlpha(1)
				x:SetSmoothing("IN")
				x:SetStartDelay(s.delay)
				x:SetDuration(0.15)
				x:SetOrder(1)
				...
				x:SetToAlpha(0.25)
				x:SetSmoothing("OUT")
				x:SetStartDelay(s.delay + s.duration - 0.15)
				x:SetDuration(0.15)
				x:SetOrder(1)
				...
				x:SetDegrees(60)
				x:SetStartDelay(s.delay)
				x:SetDuration(s.duration)
				x:SetOrder(1)

				x = a.anim:CreateAnimation("Scale")
				x:SetTarget(animTarget)
				x:SetScale(s.scale, s.scale)
				x:SetStartDelay(s.delay)
				x:SetDuration(0.25)
				x:SetOrder(1)
				x:SetSmoothing("IN")
				...
				x:SetTarget(animTarget)
				x:SetScale(0.1, 0.1)
				x:SetStartDelay(s.delay + s.duration - 0.25)
				x:SetDuration(0.25)
				x:SetOrder(1)
				x:SetSmoothing("OUT")
		...
		for i = 1, 8 do
			...
			local tex = a.sparkleTextures[i]
			tex:SetAllPoints(a.frame)
			tex:Show()
		end
		bar.sparkleEffect = a
	end
...
function MOD.Nest_SetBarGroupBarLayout(
	bg,
	barWidth,
	barHeight,
	iconSize,
...
function MOD.Nest_SetBarGroupSegments(
	bg,
	count,
	override,
	spacing,
...
function MOD.Nest_CreateBar(bg, name)
	...
	if n then
		...
		bar.cooldown:SetDrawEdge(Raven.db.global.IconClockEdge) -- added in WoD
...
function MOD.Nest_DeleteBar(bg, bar)
	...
	if config.bars == "timeline" and bg.tlSplash then
		BarGroup_TimelineAnimation(bg, bar, config)
	end
...
function MOD.Nest_StartTimer(bar, timeLeft, duration, maxTime)
	...
	bar.maxTime = maxTime or duration
	bar.expireDone = nil
	bar.warningDone = nil
	bar.update = true
end
...
function MOD.Nest_SetSparkle(bar, sparkle)
	bar.sparkle = sparkle
end

-- Set whether the bar should trigger a pulse effect
function MOD.Nest_SetPulse(bar, pulse)
	bar.pulse = pulse
end
...
local function BarGroup_UpdateAnchor(bg, config)
	...
	if not bg.locked and not inPetBattle then
		bg.anchor:Show()
	else
		bg.anchor:Hide()
	end
...
local function Bar_UpdateSettings(bg, bar, config)
	...
	if bg.showIcon and not isHeader then
		...
		if bar.sparkle then
			SparkleEffect(bar, bat.sparkleColor)
		end -- trigger sparkle animation
		if bar.pulse then
			PulseEffect(bar)
...
function MOD.Nest_UpdatePixelScale(displayReport)
	if MOD.db.global.AdjustUIScale then -- option to adjust "UIscale" for optimized pixel perfect alignment on monitor
		...
		if not MOD.db.global.SilentUIScale and displayReport then
			print(
				"Raven: detected display resolution "
					.. tostring(pixelWidth)
					.. "x"
					.. tostring(pixelHeight)
					.. ", adjusted UI scale from "
					.. tostring(oscale)
					.. " to "
					.. tostring(pscale)
			)
