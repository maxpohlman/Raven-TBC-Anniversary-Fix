...
local function CheckTestAND(ttype, t)
	...
	elseif ttype == "Spell Ready" then
		if not CheckSpellReady(t.spell, "target", not t.notUsable, t.checkCharges, t.charges) then
			return false
		end
	...
	elseif ttype == "Item Ready" then
		if not CheckItemReady(t.item, t.toggle, t.checkCount, t.count, t.checkCharges, t.charges) then
			return false
		end
	else
		return false
...
local function CheckTestOR(ttype, t)
	...
	if ttype == "Player Status" then
		...
		if IsOn(t.checkEssence) and IsOn(t.minEseence) and (t.checkEssence == (stat.essence >= t.minEssence)) then
			return true
		end
		if IsOn(t.checkInsanity) and IsOn(t.minInsanity) and (t.checkInsanity == (stat.insanity >= t.minInsanity)) then
			return true
	...
	elseif ttype == "Pet Status" then -- pet must exist for these tests to be true
		...
		if IsOn(t.checkHealth) and IsOn(t.minHealth) and (not stat.noPet and (t.checkHealth == (stat.petHealth >= t.minHealth))) then
			return true
		end
		if IsOn(t.checkPower) and IsOn(t.minPower) and (not stat.noPet and (t.checkPower == (stat.petPower >= t.minPower))) then
			return true
		end
	...
	elseif ttype == "Target Status" then -- target must exist for these tests to be true
		...
		if not stat.noTarget then
			...
			if IsOn(t.classify) and t.classification and (t.classification ~= "") and CheckClassification(stat.targetClassification, t.classification) then
				return true
			end
	...
	elseif ttype == "Target's Target Status" then -- target's target must exist for these tests to be true
		...
		if not stat.noTargetTarget then
			...
			if IsOn(t.classify) and t.classification and (t.classification ~= "") and CheckClassification(stat.targetTargetClassification, t.classification) then
				return true
			end
	...
	elseif ttype == "Focus Status" then -- focus must exist for these tests to be true
		...
		if not stat.noFocus then
			...
			if IsOn(t.classify) and t.classification and (t.classification ~= "") and CheckClassification(stat.focusClassification, t.classification) then
				return true
			end
	...
	elseif ttype == "Focus's Target Status" then -- focus's target must exist for these tests to be true
		...
		if not stat.noFocusTarget then
			...
			if IsOn(t.classify) and t.classification and (t.classification ~= "") and CheckClassification(stat.focusTargetClassification, t.classification) then
				return true
			end
	...
	elseif ttype == "Spell Ready" then
		if CheckSpellReady(t.spell, "target", not t.notUsable, t.checkCharges, t.charges) then
			return true
		end
	elseif ttype == "Spell Casting" then
	...
	elseif ttype == "Item Ready" then
		if CheckItemReady(t.item, t.toggle, t.checkCount, t.count, t.checkCharges, t.charges) then
			return true
		end
	end
	return false
...
function MOD:UpdateConditions()
	...
	if MOD.myClass == "PALADIN" then
		stat.holyPower = UnitPower("player", Enum.PowerType.HolyPower)
	else
		stat.holyPower = 0
	end
	if MOD.myClass == "EVOKER" then
		stat.essence = UnitPower("player", Enum.PowerType.Essence)
	else
		stat.essence = 0
	end
	if MOD.myClass == "WARLOCK" then
		stat.shards = UnitPower("player", Enum.PowerType.SoulShards)
	else
		stat.shards = 0
	end
	if MOD.myClass == "PRIEST" then
		stat.insanity = UnitPower("player", Enum.PowerType.Insanity)
	else
		stat.insanity = 0
	end
	if MOD.myClass == "MONK" then
		stat.chi = UnitPower("player", Enum.PowerType.Chi)
	else
		stat.chi = 0
	end
	if MOD.myClass == "SHAMAN" then
		stat.maelstrom = UnitPower("player", Enum.PowerType.Maelstrom)
	else
		stat.maelstrom = 0
	end
	if MOD.myClass == "MAGE" then
		stat.arcane = UnitPower("player", Enum.PowerType.ArcaneCharges)
	else
		stat.arcane = 0
	end
	if (MOD.myClass == "DRUID") and IsSpellKnown(78674) then -- Balance Druid special case for mana as power plus separate lunar Power
		m = UnitPowerMax("player", Enum.PowerType.Mana)
		if m > 0 then
			stat.power = (100 * UnitPower("player", Enum.PowerType.Mana) / m)
		else
			stat.power = 0
		end
		m = UnitPowerMax("player", Enum.PowerType.LunarPower)
		if m > 0 then
			stat.lunarPower = (100 * UnitPower("player", Enum.PowerType.LunarPower) / m)
		else
			stat.lunarPower = 0
		end
	else
	...
	if ct then
		...
		if MOD.ExpansionIsOrAbove(LE_EXPANSION_CATACLYSM) and UnitHasVehicleUI("player") then
			return
		end
		...
		for _, tt in pairs(MOD.testOrder) do -- loop through tests in right order
			...
			if t and t.enable then -- check if a test of this type is defined and enabled
				...
				if tt == "Player Status" then
					...
					if IsOn(t.checkHolyPower) and t.minHolyPower then
						local x = "<"
						if t.checkHolyPower then
							x = ">="
						end
						a = a .. d .. L["Holy Power String"](x, t.minHolyPower)
						d = ", "
					end
					if IsOn(t.checkEssence) and t.minEssence then
						local x = "<"
						if t.checkEssence then
							x = ">="
						end
						a = a .. d .. L["Essence String"](x, t.minEssence)
						d = ", "
					end
					if IsOn(t.checkShards) and t.minShards then
						local x = "<"
						if t.checkShards then
							x = ">="
						end
						a = a .. d .. L["Shards String"](x, t.minShards)
						d = ", "
					end
					if IsOn(t.checkArcane) and t.minArcane then
						local x = "<"
						if t.checkArcane then
							x = ">="
						end
						a = a .. d .. L["Arcane Charges String"](x, t.minArcane)
						d = ", "
					end
					if IsOn(t.checkInsanity) and t.minInsanity then
						local x = "<"
						if t.checkInsanity then
							x = ">="
						end
						a = a .. d .. L["Insanity String"](x, t.minInsanity)
						d = ", "
					end
					if IsOn(t.checkMaelstrom) and t.minMaelstrom then
						local x = "<"
						if t.checkMaelstrom then
							x = ">="
						end
						a = a .. d .. L["Maelstrom String"](x, t.minMaelstrom)
						d = ", "
					end
					if IsOn(t.checkLunarPower) and t.minLunarPower then
						local x = "<"
						if t.checkLunarPower then
							x = ">="
						end
						a = a .. d .. L["Lunar String"](x, t.minLunarPower)
						d = ", "
					end
					if IsOn(t.checkChi) and t.minChi then
						local x = "<"
						if t.checkChi then
							x = ">="
						end
						a = a .. d .. L["Chi String"](x, t.minChi)
						d = ", "
					end
				...
				elseif tt == "Spell Ready" then
					if t.spell and t.spell ~= "" then
						...
						if t.checkCharges == true then
							a = a .. d .. L["Less Than"] .. " " .. dcount .. " " .. L["Charges"]
						elseif t.checkCharges == false then
							a = a .. d .. dcount .. " " .. L["Or More"] .. " " .. L["Charges"]
						end
					end
