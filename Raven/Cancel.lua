...
function MOD:UpdateInCombatBar()
	if MOD.ExpansionIsOrAbove(LE_EXPANSION_MISTS_OF_PANDARIA) and C_PetBattles.IsInBattle() then
		MOD:HideInCombatBar()
		return
	end -- hide when entering pet battles
...
local function Overlay_OnEnter(b)
	if b.aura_id then
		...
		elseif b.aura_tt == "buff" then
			if not MOD:GetAuraData("player", b.aura_id, "HELPFUL") then
				return
			end
...
function MOD:SetInCombatBar(enable, lock, list, opt)
	...
	if g.count > 0 then
		...
		if opt.anchorFrame and GetClickFrame(opt.anchorFrame) then -- use relative positioning
			...
			g.frame:SetPoint(opt.anchorPoint or "CENTER", opt.anchorFrame, opt.anchorPoint or "CENTER", opt.anchorX, opt.anchorY)
	...
	if g and g.opt and g.initialized then -- make sure have valid layout
		...
		for i = 1, g.count do
			...
			if buff then
				...
				if g.opt.pulseStart and not v.startTime then
					v.anim:Play()
					v.startTime = GetTime()
				end
				local alpha = 1
				if duration and duration > 0 then
					...
					if g.opt.pulseEnd and (timeLeft < 0.45) and not v.anim:IsPlaying() then
						v.anim:Play()
					end
